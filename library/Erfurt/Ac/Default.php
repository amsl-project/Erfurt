...
class Erfurt_Ac_Default
{
     ...
     */
    public function isModelAllowed($type, $modelUri, $ignoreArchiveMode = false)
    {
        ...
        $user = $this->_getUser();
        $userRights = $this->_getUserModelRights($user->getUri(), $ignoreArchiveMode);
        $type = strtolower($type);
...
// TODO set the right cache tags, such that cache is invalidated!!!
     ...
     */
    private function _getUserModelRights($userURI, $ignoreArchiveMode = false)
    {
        if (!isset($this->_userRights[$userURI]) || $ignoreArchiveMode) {
            ...
            if ($result = $this->_sparql($this->_acModelUri, $sparqlQuery)) {
                $this->_filterAccess($result, $userRights, $ignoreArchiveMode);
            }
     ...
     */
    private function _filterAccess($resultList, &$userRights, $ignoreArchiveMode = false)
    {
...
#var_dump($resultList);
        ...
        // in case of enabled archive mode editing of models is prohibited
        if ($_SESSION['ONTOWIKI']['archive'] != '' && $ignoreArchiveMode == false) {
            $userRights['userAnyModelEditAllowed'] = false;
            $userRights['denyModelEdit'] = array();

            $store = Erfurt_App::getInstance()->getStore();
            $models = $store->getBackendAdapter()->getAvailableModels();

            foreach($models as $model){
                array_push($userRights['denyModelEdit'], array_keys($model)[0]);
            }
        }
